...
interface SingleSoundState {
    isPlaying: boolean;
    myVol: number;
}

export class SingleSoundtrack extends Component<SingleSoundProps, SingleSoundState> {
    private audioTag: HTMLAudioElement;

    constructor(props: SingleSoundProps) {
        super(props);
        ...
        return (
            <div>
                <button className="playBtn" onClick={this.togglePlay}>
                    {this.state.isPlaying ? "soundtrackOn" : "soundtrackOff"}
                </button>
                <input
                    type="range"
                    min={0}
